...
######################################################
######################################################
    def change_gains(self,config, level):
        rospy.loginfo("Got reconfigure request!")
        rospy.loginfo("Kp_scale:%0.3f Kp:%0.3f",(config.Kp_scale),(config.Kp))
    	self.Kp = config.Kp * config.Kp_scale *10;
  	self.Ki = config.Ki * config.Ki_scale *10;
  	self.Kd = config.Kd * config.Kd_scale *10;
    	return config
    ...
    #####################################################
        ...
        self.srv = Server(PidConfig, self.change_gains)
        ...
        self.ticks_per_meter = rospy.get_param('ticks_meter', 2445)#14860 default 1223 for our wheel
    ...
    #####################################################
        ...
        rospy.loginfo(" Derivative dt=%0.3f error=%0.3f" % (self.derivative, self.error))
